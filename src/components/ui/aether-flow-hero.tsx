...
const AetherFlowHero = ({ children, className, theme }: AetherFlowHeroProps) => {
    ...
    React.useEffect(() => {
        ...
        function init() {
            ...
            for (let i = 0; i < numberOfParticles; i++) {
                ...
                let color = isDark ? 'rgba(191, 128, 255, 0.8)' : 'rgba(115, 66, 226, 0.8)';
                ...
                for (let b = a; b < particles.length; b++) {
                    ...
                    if (distance < (canvas.width / 7) * (canvas.height / 7)) {
                        ...
                        if (isCloseToMouse) {
                             ...
                             ctx.strokeStyle = isDark ? `rgba(200, 150, 255, ${opacityValue})` : `rgba(115, 66, 226, ${opacityValue})`;
                        }
        ...
        const animate = () => {
            ...
            // Set the background color inside the canvas draw loop based on theme
            ctx.fillStyle = isDark ? 'black' : '#F9FAFB';
    ...
    return (
        <div className={cn("relative w-full overflow-hidden min-h-screen bg-white dark:bg-black transition-colors duration-300", className)}>
            ...
            <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full block pointer-events-none"></canvas>
